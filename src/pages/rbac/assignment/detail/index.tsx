...
import { useGetItemAssignmentQuery, useAssignUserMutation, useUnassignUserMutation } from "@/hooks/useRbac.query";
...
const DetailAssignmentPage = () => {
  const { id } = useParams();
  const { data, refetch } = useGetItemAssignmentQuery({ id: id! });
  const { mutate: assignMutate } = useAssignUserMutation();
  const { mutate: unassignMutate } = useUnassignUserMutation();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">User Assignment: {id}</h1>
        <p className="text-muted-foreground">Assign/unassign role pada user ini</p>
      </div>
