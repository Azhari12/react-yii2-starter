...
import { useGetPermissionByIdQuery, useAssignPermissionMutation, useUnassignPermissionMutation } from "@/hooks/useRbac.query";
...
const DetailPermissionPage = () => {
  const { id } = useParams();
  const { data, refetch } = useGetPermissionByIdQuery({ id: id! });
  const { mutate: assignMutate } = useAssignPermissionMutation();
  const { mutate: unassignMutate } = useUnassignPermissionMutation();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Permission: {id}</h1>
        <p className="text-muted-foreground">Assign/unassign items pada permission ini</p>
      </div>
